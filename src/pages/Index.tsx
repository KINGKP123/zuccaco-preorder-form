...
const Index = () => {
  ...
  const [preOrderData, setPreOrderData] = useState<PreOrderData>({
    ...
    venueType: '',
    location: '',
    dinnerNapkins: { quantity: '', customAmount: '' },
    kitchenRolls: { quantity: '', customAmount: '' },
    compressedTowels: { quantity: '', customAmount: '' },
    notes: ''
  });

  const steps = [
    <IntroStep key="intro" onNext={() => setCurrentStep(1)} />,
    <BusinessDetailsStep 
      key="business" 
      data={preOrderData} 
      onUpdate={setPreOrderData}
    ...
    />,
    <ConfirmationStep key="confirmation" data={preOrderData} />
  ];
